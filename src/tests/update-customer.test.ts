  ...
  it("should return 400 if validation fails", async () => {
    ...
    expect(res.json).toHaveBeenCalledWith({
      ...
      errors: expect.arrayContaining([
        ...
        { field: "phone", message: "Phone must be at least 11 characters long" },
        { field: "email", message: "Invalid email format" },
        { field: "document", message: "Document must be at least 9 characters long" },
      ]),
    });
  });

  it("should return 200 and updated customer if valid", async () => {
    const customerData = { name: "Matheus", phone: "12345678901", email: "matheus@example.com", document: "123456789" };
